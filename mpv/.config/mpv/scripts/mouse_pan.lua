...
local function drag_to_pan_handler(table)
	...
	if table["event"] == "down" then
		...
		if not move_up and not move_lateral then
			return
		end
		local idle = function()
			if moved then
				...
				mp.command(
					"no-osd set video-pan-x " .. clamp(pX, -3, 3) .. "; no-osd set video-pan-y " .. clamp(pY, -3, 3)
				)
				moved = false
			end
		end
		mp.register_idle(idle)
		mp.add_forced_key_binding("mouse_move", "image-viewer-mouse-move", function()
			moved = true
		end)
		cleanup = function()
...
local function cursor_centric_zoom_handler(amt)
	...
	if
		not opts.cursor_centric_zoom_dezoom_if_full_view
		and zoom_inc < 0
		and video_size[1] + diff_width + 2 * margin <= dim.w
		and video_size[2] + diff_height + 2 * margin <= dim.h
	then
...
local function align_border(x, y)
	...
	if x then
		command = command
			.. "no-osd set video-pan-x "
			.. clamp(-x * (dim.ml + dim.mr) / (2 * video_size[1]), -3, 3)
			.. ";"
	end
	if y then
		command = command
			.. "no-osd set video-pan-y "
			.. clamp(-y * (dim.mt + dim.mb) / (2 * video_size[2]), -3, 3)
			.. ";"
	end
	if command ~= "" then
		mp.command(command)
...
local function pan_image(axis, amount, zoom_invariant, image_constrained)
	...
	if image_constrained == "yes" then
		...
		local margin = (axis == "x" and amount > 0) and dim.ml
