...
local APPLICATIONS = {
	...
	{ name = "kitty", icon = "/Applications/kitty.app/Contents/Resources/kitty.icns" },
...
local function handleKeyEvent(event)
	...
	local isAltDown = event:getFlags().cmd

	-- 按下了 alt + tab 后显示菜单
	if type == hs.eventtap.event.types.keyDown and keyCode == hs.keycodes.map.tab and isAltDown then
		handleShowMenu()
	...
	-- 松开了 alt 后隐藏菜单
	if type == hs.eventtap.event.types.flagsChanged and keyCode == hs.keycodes.map.cmd and not isAltDown then
		handleHideMenu()
	end
